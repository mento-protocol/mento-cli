...
export default class ListPools extends BaseCommand<typeof BaseCommand> {
  ...
  async run() {
    const { flags } = await this.parse(ListPools);
  ...
  private renderSimpleList(exchanges: [string, [string, string]][], tokenInfo: string[]) {
    for (const [i, [poolId]] of exchanges.entries()) {
      ...
      this.log(`${chalk.cyan(poolId)} - ${chalk.green(token0Symbol)}/${chalk.green(token1Symbol)}`);
    }
  }
}
